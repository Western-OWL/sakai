...
// Future versions of Sakai use lang3 --bbailla2
	...
	public PhaseStatus validatePhase(Phase phase, PublishedAssessmentIfc assessment, HttpServletRequest request) {
		...
		switch (phase)
		{
			case ASSESSMENT_REVIEW:
				success = getInstructorReviewUrl(assessment.getPublishedAssessmentId(), currentAgentId).isPresent();
				break;
			default:
				success = getAlternativeDeliveryUrl(assessment.getPublishedAssessmentId(), currentAgentId).isPresent();
		}
		return success ? SecureDeliveryServiceAPI.PhaseStatus.SUCCESS : SecureDeliveryServiceAPI.PhaseStatus.FAILURE;
	}
        ...
        if (statusCode == 200 && r.length() > 100) {
          try {
            ...
            if (root.isArray()) {
              ...
              if (!StringUtils.isBlank(studentUrl) && !StringUtils.isBlank(instructorUrl)) {
