...
<!--jsp/discussionForum/message/dfAllMessages.jsp-->
	...
	<h:form id="msgForum" rendered="#{!ForumTool.selectedTopic.topic.draft || ForumTool.selectedTopic.topic.createdBy == ForumTool.userId}">
		...
		<h:outputText styleClass="sak-banner-error" value="#{msgs.cdfm_postFirst_warning}" rendered="#{ForumTool.selectedTopic != null && ForumTool.needToPostFirst}"/>
		...
		<div id="checkbox" class="table-responsive">
			<mf:hierDataTable styleClass="#{ForumTool.selectedTopic.isMovePostings && ForumTool.topicHasVisibleMessages() ? 'table-hover allMessages' : 'table-hover allMessages hideMoveCol'}" id="messagesInHierDataTable" rendered="#{!empty ForumTool.messages}"  value="#{ForumTool.messages}" var="message" expanded="#{ForumTool.expanded}"
				...
				<h:column id="_messageCount">
					...
					<h:outputText id="topic_msg_count58" value="#{message.childCount + 1} #{msgs.cdfm_lowercase_msg}" rendered="#{message.depth == 0 && message.childCount ==0}" />
					<%-- // display singular ('message') if total message is 0 or more than 1--%>
					<h:outputText id="topic_msg_count59" value="#{message.childCount + 1} #{msgs.cdfm_lowercase_msgs}" rendered="#{message.depth == 0 && message.childCount >=1}" />
				</h:column>
